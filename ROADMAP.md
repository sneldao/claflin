...
| Jesse Livermore / Solana | Second desk — seated paper desk open (default; set `NEXT_PUBLIC_JESSE_PAPER_ENABLED=false` to close). xStock catalog + Jupiter Metis quotes + v2 paper + controller + seated UI + ElevenLabs ConvAI line. Night/Direct presentation toggle on the seated desk. PreStocks secondary duplex (issuer mark vs tokenPrice, evidence-only). Pyth comparison API honest-unavailable until Pro entitlement + unit basis. R2 live still gated. `/night-desk` remains the fixture study. See [docs/JESSE_DESK.md](docs/JESSE_DESK.md) and [build plan](docs/STOCKLANA_BUILD_PLAN.md). |
