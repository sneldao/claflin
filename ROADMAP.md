...
- `/night-desk` is a public, noindex fixture prototype shipped in `decace0`: 3D room, scripted Jesse dialogue, fictional comparison/quotes and one tab-local example record. It is not the seated Jesse paper desk. `/` defaults to Hetty/Base; Jesse opens from the house directory ([docs/JESSE_DESK.md](docs/JESSE_DESK.md)).
- House sequence (Isabel / Arbitrum) lives in §5. It is strategy, not first-page IA. Isabel and Arbitrum stay closed rooms: no quote, no paper file, no live order, and no transplanted approval. Jesse’s Solana paper desk is open. A four-card grid must not return.
...
| Jesse Livermore / Solana | Second desk — seated paper desk open (default; set `NEXT_PUBLIC_JESSE_PAPER_ENABLED=false` to close). xStock catalog + Jupiter Metis quotes + v2 paper + controller + seated UI (`JesseDeskSurface` / ticket / ledger / evidence / typed command bar). Pyth comparison API honest-unavailable until Pro key + unit basis. Conversational AssemblyAI line scaffolded (`JesseCall` + token route); `voice` capability stays null until a real session works. R2 live still gated. Night Desk remains the fixture study. See [docs/JESSE_DESK.md](docs/JESSE_DESK.md) and [build plan](docs/STOCKLANA_BUILD_PLAN.md). |
