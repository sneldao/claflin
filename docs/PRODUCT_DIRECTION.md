...
Today `/` defaults to the working Hetty/Base desk. Jesse’s seated Solana paper desk opens from the house directory on the same `/` shell. `/night-desk` is the public, noindex, scripted Jesse/Solana experience study shipped in `decace0`; it has fictional prices/quotes, optional browser speech, no microphone or provider/trading connection, and one explicitly kept example record in tab-only sessionStorage. It is not the integrated Jesse desk. [Jesse desk](JESSE_DESK.md) describes the seated product; [Stocklana Build Plan](STOCKLANA_BUILD_PLAN.md) owns remaining integration gates and the September 25 deadline.
...
| Jesse Livermore | Price action, timing, market structure, disciplined speculation. | Solana xStocks, Jupiter and Pyth are the selected integration direction; exact coverage/access remain verification gates. | Seated paper desk open from the house directory (Jupiter quotes, v2 local records, ConvAI line, Night/Direct toggle). PreStocks secondary duplex is evidence-only. Pyth comparison stays honest-unavailable without Pro entitlement. `/night-desk` remains the spatial study. Live Solana execution still gated. See [Jesse desk](JESSE_DESK.md). |
