...
**Status:** Canonical product cutover approved, 2026-09-05; seated working desk first pass, 2026-09-09; desk isolation and outcome evidence, 2026-09-09; rail-neutral offering-led house entry implemented; house presence grammar (voice-first Room, blank slip until intent, shared line foot) on foyer / Hetty / Jesse, 2026-09-23; lean desk — one boundary per decision point (duplicate `title=` removed: ModeStamp/tape/wire/ledger/dictation/call-foot, `BLANK_SLIP_NOTE` trimmed so foyer owns the ring pitch), 2026-09-23 (91ce5d6). The old onboarding/directory approach is retired. `/` is Claflin's front door: a fresh visit opens the foyer and house book; an instruction resolves to concrete offerings; only eligible desks can open the selected offering. Hetty covers Coinbase Tokenized Stocks on Base; Jesse covers Backed xStocks on Solana. Live settlement exists only where the selected desk supports it and remains env-gated. Visiting a planned desk is a closed room, not a second trading surface. An optional Sign in control may appear when Privy is configured — it does not gate the desk and is not live access. Stocklana judges should open `/?desk=jesse&view=room`. Browser and responsive QA of the seated pass remain a release checklist item, not source-test evidence.
...
4. **Say the boundary once, clearly, where it matters.** Paper/live mode is stated once — `MODE_HINTS` behind the ModeStamp ⓘ and `LINE_FOOT` at the line foot — plus at the consequential action (brass rule / lamp / thud with a one-time caption). Do not repeat "not a quote / nothing moves / reference, not your estimate" in every `title=`, tooltip, caption, hover or aria-label. Since 91ce5d6 the tape/wire ModeStamp carry no `title=`; the blank slip no longer re-sells the ring (foyer owns it). One clear line beats sixteen defensive ones.
...
9. **Paper is the house language.** The blotter is the draft; a returned estimate is a quotation slip; saved work is a ledger; watched marks are pigeonholes; the ticker is labelled reference, never a fabricated screen. One document whose state you understand — not separate decorative animations. A commemorative desk slip is furniture beside the ledger — a kept blotter line — not a trading surface. Both desks now write the instruction as a sentence — “Buy for your account 100 USDC of AAPLx” — and each value carries where it came from: said in your words, kept from the slip, assumed by the desk, set over the call, written by hand, or carried in from the foyer, and the label is only shown while that value still stands. When a price is corrected or set aside the superseded line stays struck through on the same slip rather than disappearing.
...
11. **Signals carry state; sentences carry decisions.** Light, sound, and motion report *state* (fresh/stale, arriving/expired, filed); explicit words decide *consequences* (amounts, paper-vs-live, refusals, expiry). Tiers: irreversible or money-moving stays fully explicit, always; reversible-but-time-costly goes ambient-primary with exact words one tap away; zero-cost stays ambient-only. New signals ship dual-coded — glow *plus* caption — and the caption retires after first exposure (per-browser, `claflin.signals.v1.*`, 12s); the text equivalent stays available on tap and to assistive tech permanently. Three signals are dual-coded today (`lampCool` / `fuseDrain` / `stampThud` via `useSignalCaption` + `SIGNAL_CAPTIONS` in `lib/desk/ui-copy.ts`). Checklist for every future signal: (1) what does the user lose misreading it — money, time, or nothing; (2) is the action reversible; (3) do sound-off, motion-off, and screen-reader still get it; (4) is it learnable in one captioned exposure — if not, it is decoration and gets cut. Do not add a 4th `title=` on a signal you already dual-code.
