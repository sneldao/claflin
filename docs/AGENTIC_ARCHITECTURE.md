...
- `lib/trading/domain.ts` owns shared instruction/estimate unions registered by rail modules; `catalog.ts` and `tokenized-stocks.ts` expose Base instruments; `lib/solana/catalog.ts` exposes Solana instruments; `lib/desk/offerings.ts` binds them to product/offering eligibility. `quotes.ts` is the Base injected-reader service with mandate-owned per-desk limits; `aerodrome.ts` is read-only RPC integration; `http.ts` supplies a thin request boundary. Venue adapters register in `lib/trading/adapters.ts` (`QuoteAdapter`: `venue`, `quote`; `MarkAdapter`: `source`, `market`, `read`; `ExecutionAdapter` is the shared execution port type) — Aerodrome/Jupiter quotes and Chainlink marks are the active entries; planned desks refuse with `desk_unavailable`. Desk-aware routes `GET /api/desk/[deskId]/quote|marks` resolve venue per desk/coverage with per-desk budgets and caches; legacy `/api/stocks/*` delegate to Hetty's adapters.
