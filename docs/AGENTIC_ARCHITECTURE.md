...
- **Clients** arrive directly at `/`, Claflin's desk, with no onboarding or marketplace prerequisite. The first semantic thing should be the ticket or the line. Hetty is the current Base broker. The Base desk prepares estimates, stores local paper outcomes and opens a live voice session; env-gated Base execution code also exists, with release evidence separate from code presence. Jesse's public Night Desk remains a non-executing fixture study.
- **House identity** lives in `lib/house.ts`, independent of provider IDs and persisted marketplace records. Identity order is Claflin → active desk and market → broker → current instruction. Hetty/Base is first; the other desks are explicitly planned. `voiceConversationEnabled` is true; live execution is the flag-gated `live` capability on `DESK_CAPABILITIES` (single-sourced from `NEXT_PUBLIC_LIVE_EXECUTION_ENABLED`), not a static house constant.
...
The desk's live voice path is an ElevenLabs ConvAI session: `POST /api/hetty/session` mints a signed URL server-side, and Hetty's tool calls are client tools executing against the desk draft in the browser (see `components/desk/HettyCall.tsx` and `scripts/create-hetty-agent.mjs`). The historical `<elevenlabs-convai>` widget and the retained webhook orchestrator at `/api/webhooks/elevenlabs` are not part of this path — see [Voice Transport](WIDGET_ARCHITECTURE.md) for dated observations.

## Night Desk: shipped study and integration boundary

**Baseline `decace0`, 2026-09-17.** `/night-desk` is public/noindex; `/desk-study` exposes the same study only in development. `components/night-desk/NightDesk.tsx` currently uses `lib/night-desk-state.ts`, fictional `lib/night-desk-fixtures.ts`, browser speech synthesis and a single tab-local sessionStorage entry. `NightDeskScene.tsx`/`lib/night-desk-scene.ts` own renderer/camera/cleanup. None of this connects Jesse to a real Solana catalog, Pyth, Jupiter, AssemblyAI, wallet or execution service.

The integration target separates three owners: authoritative desk/controller, conversation transport, and renderer. Night/direct presentation is a local preference over the same work; it is not a capability tier, broker/network switch, or reason to restart a voice session. Keep the controller/session above renderer selection. Camera and projected labels consume stable evidence/quote/record identities, and their animation must never create financial success or renew authority.
...
[Product Direction](PRODUCT_DIRECTION.md#night-desk-progression-and-presentation) owns this behavior. [Stocklana Build Plan](STOCKLANA_BUILD_PLAN.md) §4 owns the protocol, presentation and command contracts and §5 the four-engineer ownership map. The existing numeric quote/market/transaction policies remain unchanged by the experience decision.
...
- `/` is the canonical desk; `/desk` redirects to it. It owns its palette, mark, instrument and working surfaces, rather than importing a study composition. The old directory and onboarding journey are replaced. `/desk-study` is a development-only reference. `WorkingDesk.tsx` is the mount for the seated first pass (ticket and line on one writing surface). That composition is not information architecture. Hierarchy lives in [Product Direction](PRODUCT_DIRECTION.md).
- `useTradingDesk.ts` coordinates requests, review, local records and watched marks. `TradeTicket` is the live document; `PaperLedger` is the filed home; `DeskBoard` is the watch tray only. First arrival has no preselected instrument or amount. Product details are contextual. Navigation should describe the client's work, not page chapters.
- `lib/trading/domain.ts` owns explicit amounts and estimate types; `catalog.ts` exposes immutable instruments keyed by URN grammar (`8453:0x…` canonical for Base, `base:` accepted as an alias, `sol:`/`rh:` reserved); `quotes.ts` is an injected-reader service with mandate-owned per-desk limits; `aerodrome.ts` is read-only RPC integration; `http.ts` supplies a thin request boundary. Venue adapters register in `lib/trading/adapters.ts` (`QuoteAdapter`: venue, chain, `canQuote`, verified `quote`; `MarkAdapter`: source, market, `read`; `ExecutionAdapter` stub reserves the live boundary) — Aerodrome quotes and Chainlink marks are the first entries, planned desks refuse with `desk_unavailable`. Desk-aware routes `GET /api/desk/[deskId]/quote|marks` resolve venue per desk with per-desk budgets and caches; legacy `/api/stocks/*` delegate to Hetty's adapters. Older quote exports delegate to the hardened service where applicable.
- `GET /api/stocks/quote` accepts only `instrumentId`, `side`, `amount`, `unit`. Buy uses USDC spend; sell uses token quantity. Decimal strings convert directly to integers. Legacy `sizeUsd` requests are rejected. The desk-aware `GET /api/desk/[deskId]/quote` takes the same params with identical validation; the desk selects the venue adapter.
...
- `workflow.ts` binds manual edits and Hetty's client-tool instructions to one draft, invalidating old reviews and late responses. Her tools (`choose_instrument`, `set_instruction`, `set_amount`, `request_estimate`, `describe_desk`, `record_paper`, `cancel_instruction`, `watch_mark`, `share_desk_note`) are declared on the provisioned agent and executed in the caller's browser — draft, quote, watch, paper-record and desk-note only; no signing, submission or account authority is reachable by voice.
- `paper-records.ts` stores versioned, schema-validated local records only after review and successful storage, using quote IDs for duplicate prevention. Records preserve estimate and simulation assumptions, survive reload, and can be reused as new drafts or explicitly deleted. Storage errors block success; corrupt history is not overwritten. Records are visible to anyone using the browser profile and are not positions or call receipts. When an optional account is signed in, `usePaperSync` copies records to Redis (`/api/paper`); local storage stays authoritative; deletes are not propagated.
...
- The simulated fill equals quoted output including pool fees; additional slippage, gas, platform and call charges are zero in this model. Account balance, holdings and eligibility are not verified on the paper path. A read-only Coinbase Verifications check exists at `/api/eligibility` for a later authority tier and is not shown on the paper desk. Wallet binding, account-specific policy/pauses, bounded transaction preparation, signing, submission, reconciliation and broker tool integration remain unimplemented live-release gates.
...
The following broker-administration, data-model and CRUD sections describe retained source history. Public marketplace routes return 410 and the associated client/admin pages are retired. Do not call `GET /api/agents`, seed brokers, or configure ERC-8004 as if they were live product steps. Identity, payment and webhook code below is separate technical scaffolding, not a live trading release. Current desk routes are `/api/stocks/quote`, `/api/stocks/marks`, `/api/hetty/session`, `/api/hetty/transcript`, `/api/paper`, and `/api/eligibility`.
