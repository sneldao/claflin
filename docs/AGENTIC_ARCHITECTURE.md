...
- `useTradingDesk.ts` coordinates requests, review, local records and watched marks. `TradeTicket`, `HettyCall`, `TickerTape`, `DeskBoard` and `PaperHistory` present work, line, weather and continuity. First arrival has no preselected instrument or amount. Product details are contextual. Navigation should describe the client's work, not page chapters. The receiver still defers Three.js behind a different SVG; that is a known defect, not a performance contract.
...
- `workflow.ts` binds manual edits and Hetty's client-tool instructions to one draft, invalidating old reviews and late responses. Her tools (`choose_instrument`, `set_instruction`, `set_amount`, `request_estimate`, `describe_desk`, `record_paper`, `cancel_instruction`, `watch_mark`) are declared on the provisioned agent and executed in the caller's browser — draft, quote, watch and paper-record only; no signing, submission or account authority is reachable by voice.
