...
- **Clients** arrive directly at `/`, Claflin's desk, with no onboarding or marketplace prerequisite. The first semantic thing should be the ticket or the line. Hetty is the current Base broker. The current release prepares estimates, stores local paper outcomes, and can open a live voice session. It does not execute orders.
