...
- **House identity** lives in `lib/house.ts`, independent of provider IDs and persisted marketplace records. Identity order is Claflin → active desk and market → broker → current instruction. Hetty/Base is first; the other desks are explicitly planned. `voiceConversationEnabled` is true; `liveExecutionEnabled` is false.
