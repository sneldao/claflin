...
**Prepared:** 2026-09-16. **Updated:** 2026-09-17 after the Night Desk prototype and experience decision; same-day amendment sanctions the PreStocks secondary bounty (see contest focus). **Status:** approved integration brief; the spatial prototype is shipped, real Jesse market/voice/execution integrations are not.
...
Start from `decace0` or a descendant on `main`. The public [Night Desk prototype](https://claflin.trustfall.xyz/night-desk) is the approved spatial direction, not a completed Solana desk. `/` remains the working Hetty/Base desk. Do not replace that homepage or port Hetty's provider stack.
