...
No stock or amount is preselected. Paper records are simulations, not wallet positions or call receipts. They live in this browser first. When an optional account is configured and the client is signed in, new records are also copied to that account (best-effort; local storage stays authoritative; deletes are local-only and can reappear from the account copy). Records are visible to anyone using that browser profile.
...
| Ticket, board, tape and record presentation | `components/desk/TradeTicket.tsx`, `components/desk/DeskBoard.tsx`, `components/desk/TickerTape.tsx`, `components/desk/PaperHistory.tsx` |
